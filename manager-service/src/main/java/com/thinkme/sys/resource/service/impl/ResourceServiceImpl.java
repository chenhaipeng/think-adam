...
public class ResourceServiceImpl extends BaseServiceImpl<Resource> implements ResourceService {
    ...
    public List<Resource> findAllWithSort() {
        ...
        return query(Cnd.where("is_show", "=", 1).desc("parent_id").desc("weight"));

    }

    @Override
    public List<Menu> findMenus(User user) {
        List<Resource> resourceList = findAllWithSort();
        ...
        return convertToMenus(resourceList);
    }

    private List<Menu> convertToMenus(List<Resource> resources) {
        if (resources.size() == 0) {
...
//        组织菜单，将父子关联
    ...
    private Menu convertToMenu(Resource resource) {
        return new Menu(resource.getId(), resource.getName(), resource.getIcon(), resource.getUrl(),resource.getParentId());
    }
    ...
    private boolean hasPermission(Resource resource, Set<String> userPermissions) {
        ...
        for (String permission : userPermissions) {
            if (checkPermission(permission, actualResourceIdentity)) {
                return true;
            }
        }
    ...
    private boolean checkPermission(String permission, String actualResourceIdentity) {
