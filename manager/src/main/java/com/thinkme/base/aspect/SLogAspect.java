...
public class SLogAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("@annotation(com.thinkme.base.aspect.SLog)")
    public void webLog() {
    }
        ...
        for (Method method : methods) {
            if (method.getName().equals(methodName)) {
                ...
                if (clazzs.length == arguments.length) {
                    map.put("description", method.getAnnotation(SLog.class).description());
                    map.put("needLogin", method.getAnnotation(SLog.class).needLogin());
                    map.put("validate", method.getAnnotation(SLog.class).validate());
                    map.put("logger", method.getAnnotation(SLog.class).logger());
                    break;
                }
            }
        }
